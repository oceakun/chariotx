      ...
      try {
        ...
        if (data && data.length > 0) {
          ...
          if (isInNoida(lat, lon)) {
            ...
            alert(
              "Can't generate a path for the searched place. Only Noida is supported."
            );
          }
        } else {
      ...
      <div className='fixed top-0 left-0 right-0 z-[9999]'>
        <DraggableWrapper>
          <header className='draggable-handle w-full p-2 bg-transparent flex flex-row items-center justify-center rounded-full h-auto cursor-move'>
            <div
              className={`flex flex-row items-start justify-center gap-2 ${isDetailsOpen ? 'bg-black rounded-[15px]' : 'w-full h-full'}`}
            >
              {/* Feature Options */}
              <div
                className={`flex border border-gray-700 rounded-[15px] shadow-lg bg-[#303030] ${isDetailsOpen ? 'flex-col items-stretch py-2 px-2 gap-4' : 'flex-row justify-center items-center gap-4 px-10 py-2'} ${isDetailsOpen ? 'h-full' : ''}`}
              ...
              >
                ...
                />
                <Planner
                  onClick={() => setSelected('planner')}
                  selected={selected === 'planner'}
                />
                <Cab
                  onClick={() => setSelected('cab')}
                  selected={selected === 'cab'}
                />
                <Estimate
              ...
              {isDetailsOpen && (
                <div className='flex justify-start max-w-[300px] w-full h-full relative'>
                  ...
                  <div className='w-full'>
                    {selected === 'search' && (
                      <SearchDetails onSearch={handleSearch} />
                    )}
